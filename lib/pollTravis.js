...
function pollByCommitThenStatus (owner, repoName, prId) {
  ...
  githubClient.pullRequests.getCommits({
    ...
    if (err) {
      return logger.error(err, `! ${prInfo} Got error when retrieving GitHub commits for PR`)
    }
    ...
    logger.info(`* ${prInfo} Started polling Travis for build by commit ${lastCommit.sha.substr(0, 7)}`)
  })
}
...
function pollTravisBuildBySha (options, checkNumber) {
  ...
  if (checkNumber > 100) {
    logger.warn(`* ${prInfo} Was not able to find matching build for PR, stopping poll now :(`)
    return
  }

  travisClient.repos(options.owner, options.repoName).builds.get((err, res) => {
    if (err) {
      return logger.error(err, `! ${prInfo} Got error when retrieving Travis builds`)
    }
    ...
    if (!matchingCommit) {
      logger.warn(`! ${prInfo} Travis hasn't picked up last commit yet, will do check #${checkNumber + 1} in 30 seconds`)
    ...
    if (lastBuildForCommit) {
      ...
      if (lastState === 'passed') {
        ...
        logger.info(`* ${prInfo} "${lastState}" build found, will do check #${checkNumber + 1} in 30 seconds`)
        createGhStatus('pending', lastBuildForCommit.id, 'build in progress')
      } else {
        return logger.info(`* ${prInfo} Unknown build state: "${lastState}", stopping polling`)
      }
    } else {
      logger.warn(`! ${prInfo} Was not able to find matching build by last commit, will do check #${checkNumber + 1} in 30 seconds`)
    }
...
function createGhStatusFn (options) {
  ...
  return (state, travisId, message) => {
    githubClient.statuses.create({
      ...
      if (err) {
        return logger.error(err, `! ${prInfo} Error while updating GitHub PR status`)
      }
      logger.info(`* ${prInfo} Github PR status updated`)
    })
  }
}
