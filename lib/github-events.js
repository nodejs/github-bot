...
const isTesting = process.env.NODE_ENV === 'test' || process.env.npm_lifecycle_event === 'test'
...
module.exports = (app) => {
  app.post('/hooks/github', (req, res) => {
    ...
    // don't verify webhook secret when if we're running tests
    if (!isTesting && (!signature || signature !== sign(secret, req.raw))) {
